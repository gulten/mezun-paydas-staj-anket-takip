                ...
                >
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-toolbar-title>ÖĞRETİM ELEMANLARI</v-toolbar-title>
                            <v-divider
                                class="mx-4"
                                inset
                                vertical
                                            ...
                                            <v-container>
                                                <v-row>
                                                    ...
                                                    <v-col cols="12" sm="6" md="6">
                                                        <v-text-field
                                                            v-model="editedItem.cep_telefonu"
                                                            label="Cep Tel"
                                                            :rules="phoneRules"
                                                            v-mask="'#(###) ### ## ##'"
                                                            required
                                                    ...
                                                    <v-col cols="12" sm="6" md="6">
                                                        <v-text-field
                                                            v-model="editedItem.is_telefonu"
                                                            label="İş Tel"
                                                            :rules="phoneRules"
                                                            v-mask="'#(###) ### ## ##'"
                                                            required>
                    ...
                    <template v-slot:item.status="{ item }">
                        <v-switch
                            ...
                            false-value="passive"
                            true-value="active"
                            @change="enableDisableUser(item.user_id)">
                        </v-switch>
                    </template>
...
<script>
    export default {
        ...
        data: () => ({
            ...
            headers: [
                { text: 'ÜNVAN', value: 'unvan' },
                { text: 'AD', value: 'name' },
                { text: 'E-MAIL', value: 'email' },
                { text: 'CEP TEL', value: 'cep_telefonu' },
                { text: 'İŞ TEL', value: 'is_telefonu' },
        ...
        methods: {
            getUsers () {
                api.get('/bbys/ogretim-elemanlari')
                    .then(response => {
                        if (response.data.success) {
                            this.Items = response.data.data.ogretim_elemanlari;
                        }
                    })
            ...
            deleteItem (item) {
                if (confirm('Bu kullanıcıyı silmek istediğinize emin misiniz?')){
                    ...
                    api.delete('/bbys/ogretim-elemanlari/' + item.id).then(response => {
                        if (response.data.success) {
                            setTimeout(() => {
                                this.loader = false;
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    api.put('/bbys/ogretim-elemanlari/' + this.editedItem.id, JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
                    ...
                    let item = this.editedItem;
                    api.post('/bbys/ogretim-elemanlari', JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
