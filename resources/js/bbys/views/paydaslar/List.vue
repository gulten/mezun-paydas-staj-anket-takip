                ...
                >
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-toolbar-title>PAYDAŞLAR</v-toolbar-title>
                            <v-divider
                                class="mx-4"
                                inset
                                vertical
...
<script>
    export default {
        ...
        methods: {
            getUsers () {
                api.get('/bbys/paydaslar')
                    .then(response => {
                        if (response.data.success) {
                            this.Items = response.data.data.paydaslar;
                        }
                    })
            ...
            deleteItem (item) {
                if (confirm('Bu kullanıcıyı silmek istediğinize emin misiniz?')){
                    ...
                    api.delete('/bbys/paydaslar/' + item.id).then(response => {
                        if (response.data.success) {
                            setTimeout(() => {
                                this.loader = false;
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    api.put('/bbys/paydaslar/' + this.editedItem.id, JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
                    ...
                    let item = this.editedItem;
                    api.post('/bbys/paydaslar', JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
