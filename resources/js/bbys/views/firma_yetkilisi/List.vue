                ...
                >
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-toolbar-title>FİRMA YETKİLİSİ</v-toolbar-title>
                            <v-divider
                                class="mx-4"
                                inset
                                vertical
    ...
    export default {
        ...
        watch: {
            ...
            searchFirma (val) {
                ...
                api.get('/bbys/firmalar/ara?filter[adi]=' + val)
                .then(response => {
                    if (response.data.success) {
                        this.firmaList = response.data.data.firmalar;
                    }
                })
                .catch(err => {
                    console.log(err)
        ...
        methods: {
            getUsers () {
                api.get('/bbys/firma_yetkilisi')
                    .then(response => {
                        if (response.data.success) {
                            this.Items = response.data.data.firma_yetkilileri;
                        }
                    })
            ...
            editItem (item) {
                ...
                this.firmaList = [];
                this.firmaList[0] = this.editedItem.firma;
                this.dialog = true
            },
            ...
            deleteItem (item) {
                if (confirm('Bu kullanıcıyı silmek istediğinize emin misiniz?')){
                    ...
                    api.delete('/bbys/firma_yetkilisi/' + item.id).then(response => {
                        if (response.data.success) {
                            setTimeout(() => {
                                this.loader = false;
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    api.put('/bbys/firma_yetkilisi/' + this.editedItem.id, JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
                    ...
                    let item = this.editedItem;
                    api.post('/bbys/firma_yetkilisi', JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
