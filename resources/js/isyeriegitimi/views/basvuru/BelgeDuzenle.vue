                ...
                >
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-toolbar-title>BAŞVURU BELGELERİNİ DÜZENLE</v-toolbar-title>
                            <v-divider
                                class="mx-4"
                                inset
                                vertical
...
<script>
    export default {
        ...
        methods: {
            getDocuments () {
                api.get('/isyeriegitimi/basvuru-belgeleri/duzenle')
                    .then(response => {
            ...
            deleteItem (item) {
                if (confirm('Bu belgeyi silmek istediğinize emin misiniz?')){
                    ...
                    api.delete('/isyeriegitimi/basvuru-belgeleri/duzenle/' + item.id).then(response => {
                        if (response.data.success) {
                            setTimeout(() => {
                                this.loader = false;
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    api.post('/isyeriegitimi/basvuru-belgeleri/duzenle/' + this.editedItem.id, formData, config)
                        .then(response => {
                            if(response.data.success) {
                    ...
                    let item = this.editedItem;
                    api.post('/isyeriegitimi/basvuru-belgeleri/duzenle',  formData, config)
                        .then(response => {
                            if(response.data.success) {
                                item = response.data.data;
            ...
            enableDisableUser(id){
                api.put('/isyeriegitimi/basvuru-belgeleri/duzenle/' + id + '/toggle-status').then(response => {
                    this.color = 'success';
                    this.snackbar = true;
                    this.snackbarMessage = 'İşlem Başarılı';
                })
