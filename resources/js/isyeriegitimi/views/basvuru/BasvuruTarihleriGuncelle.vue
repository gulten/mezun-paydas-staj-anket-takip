<template>
  <div>
      <v-card flat outlined class="mb-12 grey lighten-5" min-height="200px">
        <v-card-title>
            İŞYERİ EĞİTİMİ BAŞVURU TARİHLERİNİ GÜNCELLE
        </v-card-title>
        <v-card-text>
            <v-form>
                <v-row>
...
export default {
    ...
    methods: {
        getDates () {
            api.get('/isyeriegitimi/basvuru-tarihleri/guncelle')
                .then(response => {
        ...
        let requestBody = {
            ...
            api.post('/isyeriegitimi/basvuru-tarihleri/guncelle', JSON.stringify(requestBody))
                .then(response => {
                    if (response.data.success) {
                        this.color = 'success';
                        this.snackbar = true;
