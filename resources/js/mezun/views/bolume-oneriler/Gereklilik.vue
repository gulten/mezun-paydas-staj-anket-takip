                ...
                <v-card-text class="mt-3 mb-3">
                        ...
                        <template v-slot:item.action="{ item }">
                            <v-btn
                                ...
                                :disabled="item.dersgereklilik?true:false"
                ...
                >
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-toolbar-title>GEREKLİ VE GEREKSİZ GÖRÜLEN DERSLER</v-toolbar-title>
                            <v-divider
                                class="mx-4"
                                inset
                                vertical
                                        ...
                                        <v-form v-model="dialogValid" ref="dialogform">
                                            <v-container>
                                                <v-row>
                                                    ...
                                                    <v-col cols="12">
                                                        <span>
                                                            DERSİN GEREKLİLİK SKORU (1-5 Arası)
                                                        </span>
                                                        <v-rating
                                                            v-model="editedItem.gereklilik"
                                                            :length="length"
                    ...
                    <template v-slot:item.gereklilik="{ item }">
                            <v-rating
                                v-model="item.gereklilik"
                                :length="length"
                                background-color="purple lighten-3"
                                color="purple"
                                small
    ...
    export default {
        ...
        data: () => ({
            ...
            headers: [
                ...
                { text: 'GEREKLİLİK SKORU (1-5 Arası)', value: 'gereklilik' },
        ...
        methods: {
            ...
            getOneriler () {
                api.get('mezun/ders-gereklilik')
                    .then(response => {
                        if (response.data.success) {
                            this.Items = response.data.data.dersgereklilik;
                        }
                    })
            },
            editItem (item) {
            ...
            deleteItem (item) {
                if (confirm('Bu kaydı silmek istediğinize emin misiniz?')){
                    ...
                    api.delete('/mezun/ders-gereklilik/' + item.id).then(response => {
                        if (response.data.success) {
                            setTimeout(() => {
                                this.loader = false;
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    api.put('mezun/ders-gereklilik/' + this.editedItem.id, JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
                    ...
                    let item = this.editedItem;
                    api.post('mezun/ders-gereklilik', JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
