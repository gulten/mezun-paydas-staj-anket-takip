                ...
                >
                    <template v-slot:top>
                        <v-toolbar flat color="white">
                            <v-toolbar-title>YENİ DERS ÖNERİSİ</v-toolbar-title>
                            <v-divider
                                class="mx-4"
                                inset
                                vertical
                                        ...
                                        <v-form v-model="dialogValid" ref="dialogform">
                                            <v-container>
                                                <v-row>
                                                    ...
                                                    <v-col cols="12">
                                                        <v-textarea
                                                            v-model="editedItem.on_sartlar"
                                                            label="DERS İÇİN GEREKLİ ÖN ŞARTLAR"
                                                            outlined
                                                            required>
                                                        </v-textarea>
                                                    </v-col>
                                                    <v-col cols="12">
                                                        <v-textarea
                                                            v-model="editedItem.kaynaklar"
                                                            label="DERS İÇİN KAYNAK"
                                                            outlined
                                                            required>
                                                        </v-textarea>
                                                    </v-col>
                                                    <v-col cols="12">
                                                        <v-textarea
                                                            ...
                                                            label="AÇIKLAMALAR (DERSİ ÖNERME SEBEBİNİZ)"
                                                            outlined
                                                            required>
                                                        </v-textarea>
                                                    </v-col>
    ...
    export default {
        ...
        data: () => ({
            ...
            headers: [
                ...
                { text: 'DERS İÇERİĞİ', value: 'icerik' },
                { text: 'ÖN ŞARTLAR', value: 'on_sartlar' },
                { text: 'KAYNAKLAR', value: 'kaynaklar' },
        ...
        methods: {
            getOneriler () {
                api.get('mezun/yeniders-oneri')
                    .then(response => {
                        if (response.data.success) {
                            this.Items = response.data.data.yenidersoneriler;
                        }
                    })
            },
            editItem (item) {
            ...
            deleteItem (item) {
                if (confirm('Bu kaydı silmek istediğinize emin misiniz?')){
                    ...
                    api.delete('/mezun/yeniders-oneri/' + item.id).then(response => {
                        if (response.data.success) {
                            setTimeout(() => {
                                this.loader = false;
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    api.put('mezun/yeniders-oneri/' + this.editedItem.id, JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
                    ...
                    let item = this.editedItem;
                    api.post('mezun/yeniders-oneri', JSON.stringify(this.editedItem))
                        .then(response => {
                            if(response.data.success) {
